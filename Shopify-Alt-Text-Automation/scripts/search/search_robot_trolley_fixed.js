...
const shop = 'xtendoutdoors-store'; // Fixed shop domain
  ...
  try {
    ...
    while (true) {
      ...
      if (response.status !== 200) {
        console.log(`API Error on page ${page}: Status ${response.status}`);
      ...
      console.log(`Page ${page}: Found ${response.data.products.length} products (Total: ${allProducts.length})`);
